...
df = pd.read_csv('/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/South_America/Colombia/Stations_Selected_Magdalena_v2.csv')
...
for comid, name in zip(COMIDs, Names):
	obsFiles.append(
		'/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/South_America/Colombia/Historical/Simulated_Data/ERA_5/Aggregation/Daily4/era5-sam_magdalena-aggRO-{}.csv'.format(comid))
	simFiles.append(
		'/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/South_America/Colombia/Historical/Simulated_Data/ERA_5/Aggregation/Daily5/era5-sam_magdalena-aggQ-{}.csv'.format(comid))

#User Input
catchment = 'Magdalena'
output_dir = '/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/South_America/Colombia/Historical/Simulated_Data/ERA_5/Aggregation/validationResults/'
...
plot_obs_hyd_dir = path.join(output_dir, 'ERA-5 (hourly)_Hydrographs')
if not path.isdir(plot_obs_hyd_dir):
	os.makedirs(plot_obs_hyd_dir)

plot_sim_hyd_dir = path.join(output_dir, 'ERA-5 (agg)_Hydrographs')
...
for comid, name, rio, obsFile, simFile in zip(COMIDs, Names, Rivers, obsFiles, simFiles):
	...
	plt.plot(dates_obs, obs_df.iloc[:, 0].values, 'k', color='red', label='ERA-5 (hourly) Streamflow')
	#plt.title('ERA-5 (hourly) Hydrograph for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.title('ERA-5 (hourly) Hydrograph for ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.xlabel('Date')
	...
	plt.tight_layout()
	#plt.savefig(
	#	plot_obs_hyd_dir + '/ERA-5 (hourly) Hydrograph for ' + str(id) + ' - ' + name + '. COMID - ' + str(comid) + '.png')
	plt.savefig(plot_obs_hyd_dir + '/ERA-5 (hourly) Hydrograph for ' + name + '. COMID - ' + str(comid) + '.png')
	...
	plt.figure(2)
	plt.figure(figsize=(15, 9))
	plt.plot(dates_sim, sim_df.iloc[:, 0].values, 'k', color='blue', label='ERA-5 (agg) Streamflow')
	#plt.title('ERA-5 (agg) Hydrograph for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID - ' + str(comid))
	plt.title('ERA-5 (agg) Hydrograph for ' + name + '\n River: ' + rio + '. COMID - ' + str(comid))
	plt.xlabel('Date')
	plt.ylabel('Streamflow (m$^3$/s)')
	plt.legend()
	plt.grid()
	...
	plt.tight_layout()
	#plt.savefig(
	#	plot_sim_hyd_dir + '/ERA-5 (agg) Hydrograph for ' + str(id) + ' - ' + name + '. COMID - ' + str(comid) + '.png')
	plt.savefig(plot_sim_hyd_dir + '/ERA-5 (agg) Hydrograph for ' + name + '. COMID - ' + str(comid) + '.png')
	...
	plt.figure(3)
	plt.figure(figsize=(15, 9))
	plt.plot(merged_df.index, sim_volume_cum, 'k', color='blue', label='ERA-5 (agg) Volume')
	plt.plot(merged_df.index, obs_volume_cum, 'k', color='red', label='ERA-5 (hourly) Volume')
	#plt.title('Volume Analysis for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.title('Volume Analysis for ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.xlabel('Date')
	...
	plt.savefig(volume_analysis_out_dir + '/Volume Analysis for ' + name + '. COMID - ' + str(comid) + '.png')

	hv.plot(merged_df, legend=('ERA-5 (agg)', 'ERA-5 (hourly)'), grid=True,
	...
	hv.plot(merged_data_df=daily_avg, legend=('ERA-5 (agg)', 'ERA-5 (hourly)'), grid=True, x_season=True,
	        #title='Daily Average Streamflow (Standard Error) for ' + str(
		    #    id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid),
	        title='Daily Average Streamflow (Standard Error) for ' + name + '\n River: '
	              + rio + '. COMID: ' + str(comid),
	...
	plt.savefig(path.join(daily_average_out_dir, '{0}_{1}_daily_average.png'.format(str(comid), name)))

	hv.plot(merged_data_df=daily_avg, legend=('ERA-5 (agg)', 'ERA-5 (hourly)'), grid=True, x_season=True,
	        #title='Daily Average Streamflow for ' + str(
		    #    id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid),
	        title='Daily Average Streamflow for ' + name + '\n River: ' + rio + '. COMID: ' + str(comid),
	        labels=['Datetime', 'Streamflow (m$^3$/s)'], linestyles=['b-', 'r-'], fig_size=(15, 9))
	#plt.savefig(path.join(daily_average_out_dir, '{0}_{1}_daily_average_1.png'.format(str(id), name)))
	plt.savefig(path.join(daily_average_out_dir, '{0}_{1}_daily_average_1.png'.format(str(comid), name)))

	monthly_avg = hd.monthly_average(merged_df)
	monthly_std_error = hd.monthly_std_error(merged_data=merged_df)
	hv.plot(merged_data_df=monthly_avg, legend=('ERA-5 (agg)', 'ERA-5 (hourly)'), grid=True, x_season=True,
	...
	plt.savefig(path.join(monthly_average_out_dir, '{0}_{1}_monthly_average.png'.format(str(comid), name)))

	hv.scatter(merged_data_df=merged_df, grid=True,
	           #title='Scatter Plot for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid),
	           title='Scatter Plot for ' + name + '\n River: ' + rio + '. COMID: ' + str(comid),
	           labels=('ERA-5 (agg)', 'ERA-5 (hourly)'), line45=True, best_fit=True, figsize=(15, 9))
	...
	hv.scatter(sim_array=sim_array, obs_array=obs_array, grid=True,
	           ...
	           title='Scatter Plot (Log Scale) for ' + name + '\n River: ' + rio + '. COMID: ' + str(
		           comid),
	           labels=('ERA-5 (agg)', 'ERA-5 (hourly)'), line45=True, best_fit=True, log_scale=True, figsize=(15, 9))
	...
	hv.hist(merged_data_df=merged_df, num_bins=100, legend=('ERA-5 (agg)', 'ERA-5 (hourly)'), grid=True,
	...
	hv.qqplot(merged_data_df=merged_df,
	          ...
	          xlabel='ERA-5 (agg)', ylabel='ERA-5 (hourly)', legend=True, figsize=(15, 9))
	...
	for metric in time_lag_metrics:
		...
		_, time_table = hs.time_lag(merged_dataframe=merged_df, metrics=[metric], interp_freq='1D', interp_type='pchip',
		                            ...
		                            plot_title=metric + ' at Different Lags for ' + name + '\n River: ' + rio + '. COMID: '
		                                       + str(comid), plot=True,
		                            ...
		                            save_fig=path.join(station_out_dir,
		                                               '{0}_timelag_plot_for{1}_{2}.png'.format(metric, str(comid), name))
		                            )
		plt.grid()
...
volume_df = pd.DataFrame(volume_list, columns=['Station', 'ERA-5 (hourly) Volume', 'ERA-5 (agg) Volume', 'Percent Difference'])
