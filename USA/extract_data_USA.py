...
def extract_by_rivid(rivid, folder_path, outpath):
    ...
    # Get the index for the selected rividzzz
...
if __name__ == "__main__":
    #path_to_files = r"/Volumes/BYU_HD/input/north_america-continental/"
    #path_to_files = r"/Volumes/storage/ECMWF_Gridded_Runoff_Files/output_compressed/north_america-continental/"
    path_to_files = r"Z:\ECMWF_Gridded_Runoff_Files\output_compressed\north_america-continental"
    ...
    #    if not os.path.isdir("/Users/student/Desktop/output/North_America/United_States/{0}-{1}".format(spt, name)):
    #        os.makedirs("/Users/student/Desktop/output/North_America/United_States/{0}-{1}".format(spt, name))
    #    output_path = "/Users/student/Desktop/output/North_America/United_States/{0}-{1}".format(spt, name)
    #    print(spt, comid, path_to_files, output_path)
    #    extract_by_rivid(comid, path_to_files, output_path)

    '''On Windows'''
    for name, spt, comid in zip(names, spt_id, stations):
        if not os.path.isdir('C:\\Users\\jorgessanchez7\\Desktop\output\\North_America\\United_States\\{0}-{1}'.format(spt, name)):
            os.makedirs("C:\\Users\\jorgessanchez7\\Desktop\\output\\North_America\\United_States\\{0}-{1}".format(spt, name))
        output_path = "C:\\Users\\jorgessanchez7\\Desktop\\output\\North_America\\United_States\\{0}-{1}".format(spt, name)
        print(spt, comid, path_to_files, output_path)
        extract_by_rivid(comid, path_to_files, output_path)