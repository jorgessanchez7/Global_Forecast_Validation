...
dayavg_obs_may = hydrostats.data.daily_average(observed_may, rolling=True)
...
simulated_may = simulated_values[simulated_values.index.month == 5]
simulated_may_2 = simulated_may.loc[simulated_may.index >= pd.to_datetime(dt.datetime(observed_may.index[0].year, observed_may.index[0].month, 1))]

dayavg_sim_may = hydrostats.data.daily_average(simulated_may_2, rolling=True)
...
corrected_may = corrected_values[corrected_values.index.month == 5]
corrected_may_2 = corrected_may.loc[corrected_may.index >= pd.to_datetime(dt.datetime(observed_may.index[0].year, observed_may.index[0].month, 1))]

dayavg_corr_may = hydrostats.data.daily_average(corrected_may_2, rolling=True)
...
date_range = pd.date_range(start=simulated_may_2.index.min(), end=simulated_may_2.index.max(), freq='D')
observed_may_2 = observed_may.reindex(date_range)
observed_may_2.index.name = 'Datetime'
observed_may_2 = observed_may_2[observed_may_2.index.month == 5]

# Usamos directamente el índice actual del dataframe, sin importar qué fechas son
plot_index = range(1, len(observed_may_2) + 1)

# Asignar índice artificial a cada DataFrame (todos tienen igual longitud tras reindexado)

observed_may_2 = observed_may_2.copy()
observed_may_2["plot_index"] = plot_index

simulated_may_2 = simulated_may_2.copy()
simulated_may_2["plot_index"] = plot_index

corrected_may_2 = corrected_may_2.copy()
corrected_may_2["plot_index"] = plot_index

###Station 1###
#observed_may_2 = observed_may_2.head(217)
#simulated_may_2 = simulated_may_2.head(217)
#corrected_may_2 = corrected_may_2.head(217)
#observed_may_2 = observed_may_2.iloc[93:]
#simulated_may_2 = simulated_may_2.iloc[93:]
#corrected_may_2 = corrected_may_2.iloc[93:]


observed_may_3 = observed_may_2.dropna()

# Plotting the first graph (top-left)
axs[0, 0].plot(simulated_may_2["plot_index"], simulated_may_2[retro_input], label='Simulated Streamflow', color='#EF553B')
#axs[0, 0].set_title('Simulated Hydrograph for May (2009-2025)', fontweight='bold') #Station 1
axs[0, 0].set_title('Simulated Hydrograph for May (2009-2025)', fontweight='bold') #Station 2
...
axs[0, 0].set_ylim(0, 35000)  #Station 2  # Set y-limit for the upper left plot
...
axs[0, 1].set_ylim(0, 35000)  #Station 2  # Set y-limit for the upper left plot

# Plotting the third graph (bottom-left)
axs[1, 0].plot(observed_may_3["plot_index"], observed_may_3['Water Level (m)'], label='Observed Water Level', color='#ff7f0e', linestyle='--', marker='o')
axs[1, 0].plot(corrected_may_2["plot_index"], corrected_may_2['Transformed Water Level (m)'], label='Transformed Water Level', color='#1f77b4')
#axs[1, 0].set_title('Observed and Transformed Water Level values for May (2009-2025)', fontweight='bold') #Station 1
axs[1, 0].set_title('Observed and Transformed Water Level values for May (2009-2025)', fontweight='bold') #Station 2
axs[1, 0].set_ylabel('Water Level (m)')
axs[1, 0].set_xlabel('Time')  # Adding x-label
axs[1, 0].legend()
axs[1, 0].grid(True)  # Add grid
#axs[1, 0].set_ylim(402, 406)  #Station 1  # Set y-limit for the upper left plot
axs[1, 0].set_ylim(169, 175) #Station 2  # Set y-limit for the upper left plot
...
#axs[1, 1].set_ylim(402, 406)  #Station 1  # Set y-limit for the upper left plot
axs[1, 1].set_ylim(169, 175) #Station 2  # Set y-limit for the upper left plot
...
#Station 1
#point1_x = simulated_may_2["plot_index"][278]  #point in upper left plot
#point1_y = simulated_may_2['{}'.format(retro_input)][278]  #point in upper left plot
#point2_x = f_sim(point1_y)  #point in upper right plot
#point2_y = simulated_may_2['{}'.format(retro_input)][278]  #point in upper right plot
#point3_x = point2_x  #point in lower right plot
#point3_y = ((f_obs_inv(point2_x))+(corrected_may_2['Transformed Water Level (m)'][278])) / 2
#point4_x = simulated_may_2["plot_index"][278]  #point in lower left plot
#point4_y = point3_y  #point in lower left plot

#Station 2
point1_x = simulated_may_2["plot_index"][310]  #point in upper left plot
point1_y = simulated_may_2['{}'.format(retro_input)][310]  #point in upper left plot
point2_x = f_sim(point1_y)  #point in upper right plot
point2_y = simulated_may_2['{}'.format(retro_input)][310]  #point in upper right plot
point3_x = point2_x  #point in lower right plot
point3_y = ((f_obs_inv(point2_x))+(corrected_may_2['Transformed Water Level (m)'][310])) / 2
point4_x = simulated_may_2["plot_index"][310]  #point in lower left plot
point4_y = point3_y  #point in lower left plot
