...
def get_forecast_files(river_id, start_date, end_date):
	...
	while current_date <= end_date:
		...
		# Fetch forecast data for the current date
		try:
			url_1 = 'https://geoglows.ecmwf.int/api/ForecastEnsembles/?reach_id={0}&date={1}{2}{3}&ensemble=1-52&return_format=csv'.format(river_id,YYYY,MM,DD)
			s = requests.get(url_1, verify=False).content
			...
			df1.rename(columns={"ensemble_01_m^3/s": "ensemble_01", "ensemble_02_m^3/s": "ensemble_02", "ensemble_03_m^3/s": "ensemble_03", "ensemble_04_m^3/s": "ensemble_04",
								...
								"ensemble_49_m^3/s": "ensemble_49", "ensemble_50_m^3/s": "ensemble_50", "ensemble_51_m^3/s": "ensemble_51", "ensemble_52_m^3/s": "ensemble_52"}, inplace=True)

			if not os.path.isdir("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast\\{0}-{1}-{2}\\".format(YYYY, MM, DD)):
				os.makedirs("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast\\{0}-{1}-{2}\\".format(YYYY, MM, DD))

			df1.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast\\{0}-{1}-{2}\\{3}.csv".format(YYYY, MM, DD, river_id))

			if not os.path.isdir("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Stats\\{0}-{1}-{2}\\".format(YYYY, MM, DD)):
				os.makedirs("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Stats\\{0}-{1}-{2}\\".format(YYYY, MM, DD))

			forecast_stats_df = forecast_stats(df1)
			forecast_stats_df.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Stats\\{0}-{1}-{2}\\{3}.csv".format(YYYY, MM, DD, river_id))
...
def get_dwlt_forecast_files(river_id, start_date, end_date, retrospective_simulation, observed_adjusted, min_value):
	...
	while current_date <= end_date:
		...
		try:
			forecast_df = pd.read_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast\\{0}-{1}-{2}\\{3}.csv".format(YYYY, MM, DD, river_id), index_col=0)
			...
			corrected_ensembles_sbwl.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast\\{0}-{1}-{2}\\{3}_WL.csv".format(YYYY, MM, DD, river_id))
			forecast_stats_wl = forecast_stats(corrected_ensembles_sbwl)
			forecast_stats_wl.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Stats\\{0}-{1}-{2}\\{3}_WL.csv".format(YYYY, MM, DD, river_id))
		except Exception as e:
			print(e)
...
COMIDs_1 = stations_pd['COMID_v1'].astype(int).tolist()
Names = stations_pd['Station'].tolist()

for id, name, comid, comid_1 in zip(IDs, Names, COMIDs, COMIDs_1):

	print(id, ' - ', name, ' - ', comid, ' - ', comid_1)
	...
	simulated_df.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Record\\{0}.csv".format(comid_1))
	...
	simulated_streamflow = pd.read_csv('G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\Simulated_Data\\GEOGLOWS_v1\\{0}.csv'.format(comid_1), index_col=0)
	...
	rperiods_q.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Return_Periods\\{}.csv".format(comid_1))

	corrected_values = pd.read_csv('G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\DWLT\\GEOGLOWS_v1\\{0}-{1}_WL.csv'.format(id, comid_1), index_col=0)
	...
	rperiods_wl.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Return_Periods\\{}_WL.csv".format(comid_1))
	...
	fixed_records_sbwl.to_csv("G:\\My Drive\\Personal_Files\\Post_Doc\\Hydroweb\\GEOGLOWS_v1_test\\Forecast_Record\\{0}_WL.csv".format(comid_1))

	get_forecast_files(comid_1, "20250503", "20250617")
	get_dwlt_forecast_files(comid_1, "20250503", "20250617", simulated_streamflow, observed_adjusted, min_value)
