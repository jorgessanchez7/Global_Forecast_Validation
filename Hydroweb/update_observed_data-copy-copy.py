            ...
            if file.startswith('hydroprd') and file.endswith('.txt'):
                ...
                while skip < max_skip:
                    try:
                        ...
                        data = pd.read_csv(source_path, skiprows=skip, sep=' ', header=None, index_col=0)
                    ...
                    print(f"Failed to process file: {source_path} after "f"{max_skip} attempts.")
...
def compile_observed_data(hydroweb_txt_path:str, data_folder:str) -> pd.DataFrame:
    """
    Description
