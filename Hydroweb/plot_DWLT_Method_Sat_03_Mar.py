...
observed_march = observed_values[observed_values.index.month == 3]

dayavg_obs_march = hydrostats.data.daily_average(observed_march, rolling=True)
...
simulated_march = simulated_values[simulated_values.index.month == 3]
simulated_march_2 = simulated_march.loc[simulated_march.index >= pd.to_datetime(dt.datetime(observed_march.index[0].year, observed_march.index[0].month, 1))]

dayavg_sim_march = hydrostats.data.daily_average(simulated_march_2, rolling=True)
...
corrected_march = corrected_values[corrected_values.index.month == 3]
corrected_march_2 = corrected_march.loc[corrected_march.index >= pd.to_datetime(dt.datetime(observed_march.index[0].year, observed_march.index[0].month, 1))]

dayavg_corr_march = hydrostats.data.daily_average(corrected_march_2, rolling=True)

# Get Observed FDC
monObs = observed_march.dropna()
...
date_range = pd.date_range(start=simulated_march_2.index.min(), end=simulated_march_2.index.max(), freq='D')
observed_march_2 = observed_march.reindex(date_range)
observed_march_2.index.name = 'Datetime'
observed_march_2 = observed_march_2[observed_march_2.index.month == 3]

# Usamos directamente el índice actual del dataframe, sin importar qué fechas son
plot_index = range(1, len(observed_march_2) + 1)

# Asignar índice artificial a cada DataFrame (todos tienen igual longitud tras reindexado)

observed_march_2 = observed_march_2.copy()
observed_march_2["plot_index"] = plot_index

simulated_march_2 = simulated_march_2.copy()
simulated_march_2["plot_index"] = plot_index

corrected_march_2 = corrected_march_2.copy()
corrected_march_2["plot_index"] = plot_index

###Station 1###
#observed_march_2 = observed_march_2.head(217)
#simulated_march_2 = simulated_march_2.head(217)
#corrected_march_2 = corrected_march_2.head(217)
#observed_march_2 = observed_march_2.iloc[93:]
#simulated_march_2 = simulated_march_2.iloc[93:]
#corrected_march_2 = corrected_march_2.iloc[93:]


observed_march_3 = observed_march_2.dropna()

# Plotting the first graph (top-left)
axs[0, 0].plot(simulated_march_2["plot_index"], simulated_march_2[retro_input], label='Simulated Streamflow', color='#EF553B')
axs[0, 0].set_title('Simulated Hydrograph for March (2009-2025)', fontweight='bold') #Station 1
#axs[0, 0].set_title('Simulated Hydrograph for March (2009-2025)', fontweight='bold') #Station 2
...
axs[0, 1].set_title('Flow Duration Curve For The Month of March', fontweight='bold')
...
axs[1, 0].plot(corrected_march_2["plot_index"], corrected_march_2['Transformed Water Level (m)'], label='Transformed Water Level', color='#1f77b4')
axs[1, 0].set_title('Observed and Transformed Water Level values for March (2009-2025)', fontweight='bold') #Station 1
#axs[1, 0].set_title('Observed and Transformed Water Level values for March (2009-2025)', fontweight='bold') #Station 2
...
axs[1, 1].set_title('Water Level Duration Curve For The Month of March', fontweight='bold')
...
#Station 1
#point1_x = simulated_march_2["plot_index"][369]  #point in upper left plot
#point1_y = simulated_march_2['{}'.format(retro_input)][369]  #point in upper left plot
#point2_x = f_sim(point1_y)  #point in upper right plot
#point2_y = simulated_march_2['{}'.format(retro_input)][369]  #point in upper right plot
#point3_x = point2_x  #point in lower right plot
#point3_y = ((f_obs_inv(point2_x))+(corrected_march_2['Transformed Water Level (m)'][369])) / 2
#point4_x = simulated_march_2["plot_index"][369]  #point in lower left plot
...
if name == 'Amazonas_uaupes_km2380':
