...
for element in list:
    ...
    for id, comid, name, station, country, region in zip(IDs, COMIDs, Names, stations, Countries, Regions):
        ...
        corrected_sat_wl = pd.read_csv('/Volumes/GoogleDrive/My Drive/PhD/2022_Winter/Dissertation_v13/Hydroweb/data/Corrected_Data/{0}/{1}-{2}_{0}.csv'.format(element, station, comid), index_col=0)
        corrected_sat_wl.index = pd.to_datetime(corrected_sat_wl.index)
        corrected_sat_wl = corrected_sat_wl.groupby(corrected_sat_wl.index.strftime("%Y-%m-%d")).mean()
        corrected_sat_wl.index = pd.to_datetime(corrected_sat_wl.index)
        corrected_sat_wl.rename(columns={'Simulated Water Level (m)': 'Simulated Water Level (msnm)'}, inplace=True)

        # Merging the Data
        merged_df = hd.merge_data(obs_df=corrected_wl, sim_df=corrected_sat_wl)
    ...
    all_station_table.to_csv('/Volumes/GoogleDrive/My Drive/PhD/2022_Winter/Dissertation_v13/Hydroweb/data/Common_Places/validationResults/corrected_obs_corrected_sat/Metrics_corrected_obs_corrected_sat_{0}.csv'.format(element))
