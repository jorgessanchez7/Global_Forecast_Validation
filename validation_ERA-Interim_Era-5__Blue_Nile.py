...
for comid, name in zip(COMIDs, Names):
	obsFiles.append(
		'/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/Africa/Blue_Nile/Data/Historical/simulated_data/ERA_5/Daily/'
		+ str(comid) + '_' + str(name) + '.csv')
	simFiles.append(
		'/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/Africa/Blue_Nile/Data/Historical/simulated_data/ERA_Interim/'
		+ str(comid) + '_' + str(name) + '.csv')

#User Input
catchment = 'Blue_Nile'
output_dir = '/Users/student/Dropbox/PhD/2020 Winter/Dissertation_v9/Africa/Blue_Nile/Data/Historical/simulated_data/validationResults/'
...
for comid, name, rio, obsFile, simFile in zip(COMIDs, Names, Rivers, obsFiles, simFiles):
	...
	plt.plot(dates_obs, obs_df.iloc[:, 0].values, 'k', color='red', label='ERA-5 Streamflow')
	#plt.title('ERA-5 Hydrograph for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.title('ERA-5 Hydrograph for ' + name + '\n River: ' + rio + '. COMID: ' + str(comid))
	plt.xlabel('Date')
	...
	plt.tight_layout()
	#plt.savefig(
	#	plot_obs_hyd_dir + '/ERA-5 Hydrograph for ' + str(id) + ' - ' + name + '. COMID - ' + str(comid) + '.png')
	plt.savefig(plot_obs_hyd_dir + '/ERA-5 Hydrograph for ' + name + '. COMID - ' + str(comid) + '.png')
	...
	plt.figure(2)
	plt.figure(figsize=(15, 9))
	plt.plot(dates_sim, sim_df.iloc[:, 0].values, 'k', color='blue', label='ERA-Interim Streamflow')
	#plt.title('ERA-Interim Hydrograph for ' + str(id) + ' - ' + name + '\n River: ' + rio + '. COMID - ' + str(comid))
	plt.title('ERA-Interim Hydrograph for ' + name + '\n River: ' + rio + '. COMID - ' + str(comid))
	plt.xlabel('Date')
	plt.ylabel('Streamflow (m$^3$/s)')
	plt.legend()
	plt.grid()
	...
	plt.tight_layout()
	#plt.savefig(
	#	plot_sim_hyd_dir + '/ERA-Interim Hydrograph for ' + str(id) + ' - ' + name + '. COMID - ' + str(comid) + '.png')
	plt.savefig(plot_sim_hyd_dir + '/ERA-Interim Hydrograph for ' + name + '. COMID - ' + str(comid) + '.png')
...
volume_df = pd.DataFrame(volume_list, columns=['Station', 'ERA-5 Volume', 'ERA-Interim Volume', 'Percent Difference'])
