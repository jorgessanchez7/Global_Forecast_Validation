...
if __name__ == "__main__":
    ...
    df = pd.read_csv(r'C:\\Users\\water\\Documents\\input\\madeira_stations.csv')
    ...
    '''On Windows'''
    for name, spt, comid in zip(names, spt_id, stations):
        if not os.path.isdir('C:\\Users\\water\\Documents\\output\\South_America\\Brazil\\Madeira\\{0}-{1}'.format(spt, name)):
            os.makedirs("C:\\Users\\water\\Documents\\output\\South_America\\Brazil\\Madeira\\{0}-{1}".format(spt, name))
        output_path = "C:\\Users\\water\\Documents\\output\\South_America\\Brazil\\Madeira\\{0}-{1}".format(spt, name)
        print(spt, comid, path_to_files, output_path)
        extract_by_rivid(comid, path_to_files, output_path)
