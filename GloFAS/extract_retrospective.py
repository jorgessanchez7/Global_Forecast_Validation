...
nc_dir = Path(r"D:\GloFAS_Retrospective")
# Archivo con puntos (usa tus columnas: Lat_GloFAS, Lon_GloFAS)
points_path = Path(r"G:\My Drive\GEOGLOWS\Forecast_Comparison\Stations_Comparison_v1.csv")  # <-- cambia a tu ruta real
points_sep = ","  # usa "\t" si tu tabla está separada por tabulaciones
# Carpeta de salida
out_dir = Path(r"G:\My Drive\GEOGLOWS\Forecast_Comparison\Retrospective\GloFAS")
out_dir.mkdir(parents=True, exist_ok=True)

# Variable y coordenadas esperadas en GloFAS historical
VAR_NAME = "dis24"        # river_discharge_in_the_last_24_hours
LAT_NAME = "latitude"
LON_NAME = "longitude"
TIME_NAME = "time"
...
def extract_series_from_file(nc_file: str, lat: float, lon: float) -> pd.DataFrame:
    ...
    # Limpieza: fechas a YYYY-MM-DD (sin hora), negativos a 0, orden y sin duplicados
    df.index = pd.to_datetime(df.index)
    # quitar zona horaria si existiera
    if df.index.tz is not None:
        df.index = df.index.tz_convert(None)
    # dejar solo fecha (agrupa si vinieran múltiples horas por día)
    df = (
...
def latlon_to_filename(lat: float, lon: float) -> str:
    ...
    return f"{lat:.3f}_{lon:.3f}.csv".replace("+", "")

# ====== MAIN ======
def main():
    # Lee puntos
    pts = pd.read_csv(points_path, sep=points_sep)
    if not {"Lat_GloFAS", "Lon_GloFAS"}.issubset(pts.columns):
        raise ValueError("El archivo de puntos debe contener columnas 'Lat_GloFAS' y 'Lon_GloFAS'.")

    nc_files = list_nc_files(nc_dir)

    # Opcional: ordenar por año detectado en el nombre si está presente
    # (Si tus archivos se llaman, p.ej., GloFAS_YYYY.nc). Si no, el sorted() inicial suele bastar.
    # nc_files = sorted(nc_files, key=lambda p: ''.join(filter(str.isdigit, os.path.basename(p))))

    for i, row in pts.iterrows():
        lat = float(row["Lat_GloFAS"])
        lon = float(row["Lon_GloFAS"])
