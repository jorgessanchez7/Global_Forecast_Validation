...
regions = ['south_america-geoglows']
...
for region in regions:
	...
	# User Input
	catchment = 'World'
	output_dir = '/Users/ElkiGio/Desktop/ERA_5-ERA_Interim/validationResults/{0}/'.format(region)
	...
	for point in POINTs:
		...
		'''Get Era_5 Data'''
		era5_df = pd.read_csv('/Volumes/files/ECMWF_Precipitation/Time_Series/ERA_5/{0}/{1}.csv'.format(region, point), index_col=0)
		era5_df.index = pd.to_datetime(era5_df.index)
		era5_df.rename({'Precipitation (mm)': 'ERA-5 Precipitation (mm)'}, axis=1, inplace=True)

		'''Get Era_Interim Data'''
		eraI_df = pd.read_csv('/Volumes/files/ECMWF_Precipitation/Time_Series/ERA_Interim/{0}/{1}.csv'.format(region, point), index_col=0)
		eraI_df.index = pd.to_datetime(eraI_df.index)
		eraI_df.rename({'Precipitation (mm)': 'ERA-I Precipitation (mm)'}, axis=1, inplace=True)
