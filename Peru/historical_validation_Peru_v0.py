...
df = pd.read_csv('/Users/student/Github/Bias_Correction/Peru/SENAMHI-ANA_Stations_v3.csv')

IDs = df['Codigo'].tolist()
COMIDs = df['new_COMID'].tolist()
Names = df['Estacion'].tolist()
Rivers = df['Rio'].tolist()
ts_ids = df['id_Estacio'].tolist()

obsFiles = []
simFiles = []
#COD = []

for id, ts_id,comid, name in zip(IDs, ts_ids, COMIDs, Names):
	obsFiles.append('/Users/student/Github/Bias_Correction/Peru/Observed/{}.csv'.format(ts_id))
	#simFiles.append('/Users/student/Github/Bias_Correction/Peru/Simulated/{}.csv'.format(ts_id))
	simFiles.append('/Users/student/Github/Bias_Correction/Peru/Corrected/{}.csv'.format(ts_id))


#User Input
country = 'Peru'
#output_dir = '/Users/student/Github/Bias_Correction/Peru/validationResults_Simulated/'
output_dir = '/Users/student/Github/Bias_Correction/Peru/validationResults_Corrected/'
...
for id, ts_id, comid, name, rio, obsFile, simFile in zip(IDs, ts_ids, COMIDs, Names, Rivers, obsFiles, simFiles):

	if isinstance(rio, str):
		print(ts_id, id, comid, name, rio)
	elif math.isnan(rio):
		rio = ' '
		print(ts_id, id, comid, name, rio)
	else:
	...
	table = hs.make_table(merged_df,
	                      metrics=['ME', 'MAE', 'MAPE', 'RMSE', 'NRMSE (Mean)', 'NSE', 'KGE (2009)', 'KGE (2012)', 'R (Pearson)',
	                               'R (Spearman)', 'r2'], location=ts_id, remove_neg=False, remove_zero=False)
