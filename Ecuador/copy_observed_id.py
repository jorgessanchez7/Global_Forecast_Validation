...
for id, name, comid in zip(IDs, Names, COMIDs):
	...
	observed = pd.read_csv('/Volumes/GoogleDrive/My Drive/PhD/2022_Winter/Dissertation_v13/South_America/Ecuador/data/historical/Observed_Data/{}_Q.csv'.format(id), index_col=0)
	observed.index = pd.to_datetime(observed.index)
	observed.index = observed.index.to_series().dt.strftime("%Y-%m-%d")
	observed.index = pd.to_datetime(observed.index)
	observed.to_csv('/Volumes/GoogleDrive/My Drive/PhD/2022_Winter/Dissertation_v13/South_America/Ecuador/data/historical/Observed_Data/{}.csv'.format(id))