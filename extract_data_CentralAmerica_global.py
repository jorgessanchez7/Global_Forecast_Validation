...
if __name__ == "__main__":
    path_to_files = r"/Users/ElkiGio/Desktop/input/central_america-geoglows/"

    df = pd.read_csv(r'/Volumes/files/ECMWF_GEOGloWS_Streamflow/validationResults/central_america-geoglows/Tables/Table_of_all_stations_v3.csv')

    stations = df['Location'].tolist()

    '''On Mac'''
    for spt in stations:
        if not os.path.isdir("/Volumes/files/ECMWF_GEOGloWS_Streamflow/Time_Series/Forecast/central_america-geoglows/{0}".format(spt)):
            os.makedirs("/Volumes/files/ECMWF_GEOGloWS_Streamflow/Time_Series/Forecast/central_america-geoglows/{0}".format(spt))
        output_path = "/Volumes/files/ECMWF_GEOGloWS_Streamflow/Time_Series/Forecast/central_america-geoglows/{0}".format(spt)
        print(spt, "central_america-geoglows", path_to_files, output_path)
