...
for id, name, comid, folder, source in zip(IDs, Names, COMIDs, Folders, Sources):
	...
	try:
		...
		corrected_df.to_csv('E:\\GEOGloWS\\02_Corrected_Data\\v1\\{0}-{1}_Q.csv'.format(id, comid))
...
for id, name, comid, folder, source in zip(IDs, Names, COMIDs, Folders, Sources):
	...
	try:
		...
		corrected_df.to_csv('E:\\GEOGloWS\\03_Transformed_Data\\v1\\{0}-{1}_WL.csv'.format(id, comid))
	except Exception as e:
		print(e)
