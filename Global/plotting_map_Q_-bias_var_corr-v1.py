...
def plot_bias_map(ax, stations_df, title):
    ...
    #bias_values = stations_df['Bias'].to_list()
    bias_values = stations_df['Bias Corrected'].to_list()
...
def plot_variability_map(ax, stations_df, title):
    lons = stations_df['Longitude'].to_list()
    lats = stations_df['Latitude'].to_list()
    #variability_values = stations_df['Variability'].to_list()
    variability_values = stations_df['Corrected Variability'].to_list()
...
def plot_correlation_map(ax, stations_df, title):
    lons = stations_df['Longitude'].to_list()
    lats = stations_df['Latitude'].to_list()
    #correlation_values = stations_df['Correlation'].to_list()
    correlation_values = stations_df['Corrected Correlation'].to_list()
...
stations_df = pd.read_csv('Metrics_GEOGloWS_v1_Q.csv')
...
#plt.savefig('bias_var_corr_GEOGLOWS_v1_Q.png', dpi=400, bbox_inches='tight', pad_inches=0)
#plt.savefig('bias_var_corr_GEOGLOWS_v1_Q.pdf', format='pdf', dpi=400, bbox_inches='tight', pad_inches=0)
plt.savefig('bias_var_corr_GEOGLOWS_v1_Q_bc.png', dpi=400, bbox_inches='tight', pad_inches=0)
#plt.savefig('bias_var_corr_GEOGLOWS_v1_Q_bc.pdf', format='pdf', dpi=400, bbox_inches='tight', pad_inches=0)
#plt.show()