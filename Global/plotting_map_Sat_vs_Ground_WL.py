...
stations_df = pd.read_csv('/Users/grad/Github/Global_Forecast_Validation/Global/Metrics_Sat_vs_Ground_WL.csv')
...
for lon, lat, kge in zip(lons, lats, kge_values):
    ...
    ax.plot(lon, lat, marker='o', color=color, markersize=4, markeredgewidth=0.5, markeredgecolor='black', transform=ccrs.Geodetic())
...
for color, label in zip(legend_colors, legend_labels):
    ax.plot([], [], marker='o', color=color, label=label, linestyle='None', markersize=6, markeredgewidth=0.5, markeredgecolor='black')
...
#ax.text(-35, -24, f'${text_latex}$', fontsize=12, color='white', bbox=dict(facecolor='black', alpha=0.5), transform=ccrs.Geodetic())
#ax.text(0.5, 0.5, f'${text_latex}$', fontsize=12, color='black', verticalalignment='center')
ax.text(-35, -24, f'${text_latex}$', fontsize=12, color='white', bbox=dict(facecolor='black', alpha=1), transform=ccrs.Geodetic())
...
def draw_scale_bar(ax, location, length_km, projection):
    ...
    # Draw the scale bar
    ax.plot([x, x + scale_length_deg], [y, y], transform=projection, color='black', linewidth=3)
...
#plt.savefig('/Users/grad/Library/CloudStorage/Box-Box/MSc_Darlly_Rojas/2024_Winter/Updated_Plots_v2/Satellite_Data_Verification.png', dpi=400, bbox_inches='tight', pad_inches=0)
plt.savefig('/Users/grad/Library/CloudStorage/Box-Box/MSc_Darlly_Rojas/2024_Winter/Updated_Plots_v2/Satellite_Data_Verification.pdf', format='pdf', dpi=400, bbox_inches='tight', pad_inches=0)
#plt.show()